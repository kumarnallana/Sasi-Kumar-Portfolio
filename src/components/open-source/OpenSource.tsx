...
import { useGithubProfile } from "@/integrations/github/use-github-profile";
import { useGithubRepos } from "@/integrations/github/use-github-repos";
import { useGithubStarred } from "@/integrations/github/use-github-starred";
import type { GitHubRepo } from "@/integrations/github/github.client";
...
export default function OpenSource() {
  ...
  // Typed queries via integration hooks
  const { data: profile, isLoading: isProfileLoading, error: profileError } = useGithubProfile();
  const { data: repos, isLoading: isReposLoading, isError: isReposError } = useGithubRepos();
  const { data: starred, isLoading: isStarredLoading, isError: isStarredError } = useGithubStarred();

  // Calculate stats
  const totalStars = repos?.reduce((acc, r) => acc + (r.stargazers_count ?? 0), 0) ?? 0;
  ...
  const activeRepos: GitHubRepo[] | undefined = viewMode === "OWNED" ? repos : starred;
